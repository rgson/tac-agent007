 ...
 * $Revision: 1.8 $, $Date: 2003/07/10 19:53:59 $
 *
...
/**
 * This class implements a thread pool for reusing threads.
 * Since only a maximal number of threads may exist in the pool it is
 * important that the threads in the pool is not absorbed by long duration
 * tasks that drains the pool.
 *
 ...
 * @version $Revision: 1.8 $, $Date: 2003/07/10 19:53:59 $
 */
public final class ThreadPool {
  ...
  public static JobStatus getJobStatus() {
    ...
    return (thread instanceof PoolThread)
      ? (JobStatus) thread
      : null;
  }
  ...
  /**
   * Returns the status of all existing pool threads or <CODE>null</CODE>
   * if no pool threads exists.
   */
  ...
  public StringBuffer getThreadStatus(StringBuffer sb) {
    ...
    sb.append("ThreadPool ").append(getName())
      .append(" (threads=").append(count)
      .append(" idle=").append(idleThreadCount).append(')');
    if (count > 0) {
  ...
  /**
   * Sets the time after which a running job may be interrupted if it
   * still has not finished. A time of 0 (default) means the jobs can
   * run for unlimited time and will never be interrupted.
   *
   * @param millisSeconds the time after which a running job may be
   * interrupted if it still has not finished.
   */
  public void setInterruptThreadsAfter(int milliSeconds) {
    if (milliSeconds > 0) {
      int timeBetweenChecks = milliSeconds / 3;
      this.millisBetweenChecks = timeBetweenChecks < 1000
	? 1000
	: timeBetweenChecks;
    }
  ...
  /**
   * Invokes the specified job as soon as possible. New threads are
   * created if none is available in the thread pool.
   *
  ...
  public void invokeLater(Runnable job, String description) {
	...
	// All threads busy and no more threads allowed => add to job queue
	if (pendingJobs == null) {
  ...
  private PoolThread createThread(boolean isIdle) {
    ...
    if (poolThreads == null) {
      ...
      poolThreads = (PoolThread[])
	ArrayUtils.setSize(poolThreads, threadCount + 100);
    }

    String threadName = getName() + '.' + (++threadID);
    thread = poolThreads[threadCount] =
      new PoolThread(this, threadName, isIdle);
   ...
   *
   * @param thread the thread to add to the thread pool.
   * @return true if the thread was added to the pool and false if the
   *     pool was full and the thread should die
   */
  final boolean addThread(PoolThread thread) {
	...
	if (idleThreads == null) {
	  ...
	  idleThreads = (PoolThread[])
	    ArrayUtils.setSize(idleThreads, maxIdleThreads);
	}
  ...
  final void checkWorkingThreads(long currentTime) {
	...
	for (int i = 0; i < threadCount; i++) {
	  ...
	  if (pt.isWorking() && pt.addActive(1) > 3) {
	    pt.log.warning("interrupting overdue job " + pt.getStatus());
	    pt.interrupt();
