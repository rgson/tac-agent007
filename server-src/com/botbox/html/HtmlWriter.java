 ...
 * $Revision: 1.10 $, $Date: 2004/09/07 14:46:03 $
 *
 ...
 * @version $Revision: 1.10 $, $Date: 2004/09/07 14:46:03 $
 */
public class HtmlWriter {
  ...
  public HtmlWriter table(int type, String attributes) {
    ...
    if (currentTableStyle.isDoubleTable()) {
      ...
      pushMode(innerAttributes != null
	       ? (TAG_OPEN | NEWLINE | CACHE_ATTR)
	       : (TAG_OPEN | NEWLINE));
    }
    if (attributes != null) {
      attr(attributes);
  ...
  public HtmlWriter tr() {
    ...
    if (mode >= 0) {
      ...
      // Need to use a special mode to preserve the NEWLINE in the above mode
  ...
  private void td(String tag, String text, String attributes, int tagType) {
	...
	mode |= TABLE_TR;
      ...
      String tdAtts = tagType == TABLE_TH
	? currentTableStyle.getThAttributes()
	: currentTableStyle.getTdAttributes();
  ...
  public HtmlWriter h1(String text) {
    checkMode();
    return a(EOL).a(heading1Start).a(text).a(heading1End).a(EOL).addNewLine();
  }

  public HtmlWriter h2(String text) {
    checkMode();
    return a(EOL).a(heading2Start).a(text).a(heading2End).a(EOL).addNewLine();
  }

  public HtmlWriter h3(String text) {
    checkMode();
    return a(EOL).a(heading3Start).a(text).a(heading3End).a(EOL).addNewLine();
  }

  public HtmlWriter h4(String text) {
    checkMode();
    return a(EOL).a(heading4Start).a(text).a(heading4End).a(EOL).addNewLine();
  }
...
//   }
  ...
  public HtmlWriter attr(String name, String value) {
    ...
    if (mode <= 0 || (mode & PENDING) == 0) {
      ...
      System.err.println("HtmlWriter: could not add attribute outside tag: "
			 + name + "='" + value + '\'');
      return this;
    }
	...
	// Should cache this result. FIX THIS!!!
	...
	if ("width".equals(name)) {
	  ...
	  // The inner table must always have width = 100% if it should
  ...
  private void addOuterAttr(String name, String value) {
    ...
    int index = ArrayUtils.keyValuesIndexOf(outerAttrCache, 2,
					    0, size, name);
    if (index >= 0) {
	...
	outerAttrCache = (String[])
	  ArrayUtils.setSize(outerAttrCache, size + 10);
      }
  ...
  private void addInnerAttr(String name, String value) {
    ...
    int index = ArrayUtils.keyValuesIndexOf(innerAttrCache, 2,
					    0, size, name);
    if (index >= 0) {
	...
	innerAttrCache = (String[])
	  ArrayUtils.setSize(innerAttrCache, size + 10);
      }
  ...
  private void parseAttributes(String attributes, int type) {
    ...
    char c;
    String name = null;
    if (DEBUG) System.out.println("Parsing '" + attributes + '\'');
	...
	case 1:  // Name
	  if (c <= ' ') {
	    String value = isStuffed
	      ? destuff(attributes, start, i)
	      : attributes.substring(start, i);
	    isStuffed = false;
	    if (inValue) {
	      if (DEBUG) System.out.println("Attribute '" + name + "'='" + value + '\'');
	    ...
	    isStuffed = false;
	    if (inValue) {
	      // Assume that the current value really is the name of
	      // next attribute and that the previous attribute had no
	      // value.  Is this correct behaviour??? FIX THIS!!!

	      if (DEBUG) System.out.println("Attribute '" + name + "'=null");
    ...
    if (mode > 0) {
      String value = isStuffed
	? destuff(attributes, start, attributes.length())
	: attributes.substring(start);
      if (inValue) {
	// Name and value
	...
	// in name
	if (DEBUG) System.out.println("Attribute '" + value + "'=null");
  ...
  private void checkMode() {
    ...
    // Special handling inside tables (should this be done??? FIX THIS!!!)
    int mode = peekMode(TABLE);
  ...
  private int handleMode(int mode) {
	...
	// (if one of the tagset handlers sets it to 0 it will be absorbed)
	type = mode & 0xff;
    ...
    case FORM:
      ...
      break;
    default:
      System.err.println("HtmlWriter: unhandled type=" + type
			 + " mode=" + mode);
      break;
    }
    if ((mode & NEWLINE) != 0) {
