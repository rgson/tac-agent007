...
public class OnesideContinuousAuction extends Auction {

  private int minDelay = 24;
  private int randomDelay = 9;
  ...
  public OnesideContinuousAuction(Market market, int type, int day,
				  long closeTime) {
  ...
  protected void openAuction(long openTime) {
    ...
    int delay = minDelay + random.nextInt(randomDelay);
    long nextUpdate = openTime + delay * 1000;
    double startPrice = 250.0 + random.nextInt(151);
    this.upperBound = 10 + random.nextInt(81);
  ...
  protected synchronized void updateAuctionQuote(long time) {
    ...
    // Update price
    int x = 10 + (int)
      (((double) timeInGame / gameLength) * (upperBound - 10));
    price += -10 + random.nextInt(x + 11);
    // Price constraints
    if (price > 800) {
      price = 800;
    } else if (price < 150) {
